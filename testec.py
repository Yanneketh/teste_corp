...
# Rota DELETE para remover um celular pelo IMEI
@app.route("/celulares/<imei>", methods=["DELETE"])
def delete_celular(imei):
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM celulares WHERE imei = %s", (imei,))
            conn.commit()
            if cursor.rowcount > 0:
                response = make_response(jsonify({"message": f"Celular com IMEI {imei} removido com sucesso!"}), 200)
                response.headers['Access-Control-Allow-Origin'] = '*'
                return response
